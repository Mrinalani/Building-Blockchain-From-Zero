...
const MY_ADDRESS = "ws://localhost:3001"; // server address
...
console.log("John is listening on port", PORT); // log


server.on("connection", (socket) => { // socket object is the socket of node which wishes to connect john node
   console.log("[JOHN] New connection established"); // log
   ...
   socket.on("message", (message) => { // event which handle incoming messages from node
    console.log("*****************", message)
       ...
       console.log("[JOHN] Received:", _message); // log
       ...
       switch (_message.type) { // message structure => {type:..., data:...}
           case "TYPE_HANDSHAKE": // means node sending addresses it is already connected to
               ...
               console.log("[JOHN] Received handshake with nodes:", nodes); // log


               nodes.forEach(node => connect(node)); // connect to all nodes
               break;

            case "REPLACE_CHAIN": // Replace old chain with updated new chain after john mines new block
           ...
           case "MESSAGE": // means node sending addresses it is already connected to
               console.log("[JOHN] Message:", _message.data); // log
               break;
       }


       console.log("[JOHN] Connected:", connected); // log
   });
});
...
const connect = (address) => {
   if (!connected.includes(address) && address !== MY_ADDRESS) { // dont connect to node if its already in connected and if its own address
       ...
       socket.on("open", () => { // fired when connection with node is established
           console.log(`[JOHN] Connected to ${address}`); // log
           ...
           opened.forEach(peer => { // send the nodes JOHN is already connected to other node
       ...
       socket.on("close", () => { // Fired when connection with node is closed
           console.log(`[JOHN] Connection closed with ${address}`); // log
...
rl.on("line", (command) => {
    switch (command.toLowerCase()) {
        case 'send':
            const transaction = new Transaction(keys.JOHN_KEY.getPublic('hex'), keys.JENIFER_KEY.getPublic('hex'), 200, 20);
            console.log("sign", keys.JOHN_KEY);
            transaction.sign(keys.JOHN_KEY);
            ...
            case 'balance':
            console.log("John's Balance", satoshiCoin.getBalance(keys.JOHN_KEY.getPublic("hex")))
            break;
...
process.on("uncaughtException", err => console.log("[JOHN ERROR]", err)); // log error if anything goes wrong
